...
Common rules apply to every project. Edge rules apply only when a project declares them through repository instructions, a local skill, a specification, dependency/configuration evidence, or the user request.
...
Inactive/not evidenced: analytics, CMS, multi-tenant support.
```
...
| --- | --- | --- |
| i18n | `next-intl`, locale routing, message files, or explicit multilingual requirement exists | Read the local i18n rule. All user-facing strings, including aria labels and errors, use translations. Machine codes, database enums, URLs, and analytics keys are not UI copy. |
...
| Auth and security | Auth library, protected data, roles, uploads, or secrets are in scope | Authorization runs server-side for every mutation. Do not expose secrets or persist bearer tokens outside the approved session design. |
...
| Realtime | WebSocket, SSE, subscriptions, collaborative state, or event publishing is in scope | Database/server mutation is authoritative. Realtime events do not grant permission and are emitted only after a successful commit. |
...
| Testing | Test setup or explicit test requirement exists | Follow the repository test pyramid and naming. Add coverage for changed permission, state transition, and edge cases proportionate to risk. |
| Analytics | A tracking SDK, analytics spec, or event ID is provided | Do not invent event names or IDs. Require explicit event contract and consent/privacy requirements. |
...
| Backend architecture | Repository, server, database, or domain logic is in scope | Apply the project architecture. When no local architecture exists, separate domain/application/infrastructure/presentation only if complexity warrants it; do not impose layers on a simple static page. |
| Monorepo | Workspace config or multiple packages are present | Respect package ownership and dependency boundaries; do not reach across packages without the public API. |
